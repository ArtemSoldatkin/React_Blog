...
export default memo((props: CmpProps) => {
    ...
    return (
        <div className="cmp_itx">
            ...
            <textarea 
            className={`cmp_itx__input cmp_itx-${isValid !== undefined && (isValid ? 'inp_val' : 'inp_inval')}`}                
            ...
            {props.maxLength && <p className={`cmp_itx__counter cmp_itx-${isValid !== undefined && 
                (isValid ? 'cnt_val' : 'cnt_inval')}
            `}>Символов: {props.initialState && !touched ? props.initialState.length : value.length} из {props.maxLength}</p>}
            {isValid !== undefined && !isValid && props.name && <p className="cmp_itx__err">Заполните "{props.name}"!</p>}
        </div>
    )
})
