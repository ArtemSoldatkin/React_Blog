...
export default memo((props: CmpProps) => {
    ...
    useEffect(() => {
        if (props.customRule !== undefined && props.customRule !== validated) {
            props.validated !== undefined && validated === undefined
                ? setValidated(props.validated)
                : setValidated(props.customRule);
        }
    }, [props.validated, props.customRule, value, validated]);
    return (
        <div className="tx_form_ctrl">
            {props.label && <p className="tx_form_ctrl__lbl">{props.label}</p>}
            <div
                className={`tx_form_ctrl__tx_block 
      tx_form_ctrl-${validated !== undefined && (validated ? 'val' : 'inval')}`}>
                <input
                    className="tx_form_ctrl__input"
            ...
            {props.feedback && validated === false && (
                <p className="tx_form_ctrl__feedback">{props.feedback}</p>
            )}
        </div>
    );
});
