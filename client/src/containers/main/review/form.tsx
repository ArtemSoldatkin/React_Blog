...
const areEq = (pp: CmpProps, np: CmpProps) => {
    if (!userEq(pp.user, np.user) || pp.articleID !== np.articleID) return false;
    return true;
};
            ...
            update={(cache, { data }) => {
                if (data && data.addReview && data.addReview.reviews) {
                    ...
                    cache.writeFragment({
                        fragment,
                        id,
                        data: { _r, reviews, __typename: 'Article' },
                    });
                    setBody('');
                }
            }}>
            {(addReview, { loading, error }) => (
                <Loading loading={loading}>
                    <div className="new_review">
                        ...
                        <form
                            className="new_review__form"
                            onSubmit={e => handleSubmit(e, addReview)}>
                            <textarea
                                className="new_review__itx"
                                value={body}
                                onChange={e => setBody(e.target.value)}
                            />
                            <button
                                className="new_review__btn"
                                type="submit"
