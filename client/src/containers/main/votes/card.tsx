...
import { Votes, votesEq } from '../../../types';
...
const areEq = (pp: CmpProps, np: CmpProps) => {
    if (!votesEq(pp.votes, np.votes) || pp.mtn !== np.mtn || pp.id !== np.id) return false;
    return true;
};
        ...
        <GetUserID query={GET_USER_ID}>
            {({ data }) => {
                ...
                return (
                    <div>
                        <button
                            className={`vote vote-like ${my && my.value && 'vote-like_my'}`}
                            onClick={() => action(mtn, true, id)}
                            disabled={disabled}>
                            <FontAwesomeIcon icon={faThumbsUp} className="vote__icon" />
                            <p className="vote__text">{formatVote(votes, true)}</p>
                        </button>
                        <button
                            className={`vote vote-dislike ${my && !my.value && 'vote-dislike_my'}`}
                            onClick={() => action(mtn, false, id)}
                            disabled={disabled}>
