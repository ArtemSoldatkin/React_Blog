...
import { Article, InputData, articleEq } from '../../../types';
...
const areEq = (pp: CmpProps, np: CmpProps) => {
    if (!articleEq(pp.article, np.article)) return false;
    return true;
};
...
export default memo(({ article }: CmpProps) => {
    ...
    useEffect(() => {
        if (isEditing)
            setData({ title: article.title, description: article.description, body: article.body });
    }, [isEditing]);
    return (
        <>
